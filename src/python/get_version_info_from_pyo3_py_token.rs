...
pub fn get_version_info_from_pyo3_py_token<'py>(py: &'py Python) -> Result<PythonVersionInfo, Error<'py>> {
    let sys = py.import("sys")?;
    let version_info = sys.getattr("version_info")?.downcast::<PyTuple>()
